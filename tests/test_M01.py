...
def test_01_03_01():
    ...
    assert Bond.conversion_factor == 10.0, (
        "Conversion factor should be class attribute set to conversion factor from nanometers to Ångström."
    )
...
def test_01_04_01():
    ...
    assert str(bond) == expected_output, (
        f"Expected '{expected_output}' but got '{str(bond)}'"
    )
